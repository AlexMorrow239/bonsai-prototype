...
export class ErrorHandler {
  ...
  static handleServiceError(
    ...
    if (knownErrors.some((errorType) => error instanceof errorType)) {
      logger.warn(`Known error occurred during ${context}: ${JSON.stringify(logData)}`);
      throw error;
    }
    ...
    if (error instanceof MongoServerError) {
      if (error.code === 11000) {
        logger.warn(`Duplicate key error: ${JSON.stringify({ ...logData, mongoError: error.code })}`);
        throw new BadRequestException('A record with this information already exists');
      }
      logger.error(`MongoDB error: ${JSON.stringify({ ...logData, mongoError: error.code })}`);
    }

    // HTTP exceptions
    if (error instanceof HttpException) {
      logger.error(`HTTP exception: ${JSON.stringify({ ...logData, statusCode: error.getStatus() })}`);
      throw error;
    }

    // Unknown errors
    logger.error(`Unhandled error: ${JSON.stringify({ ...logData, severity: 'CRITICAL' })}`);
